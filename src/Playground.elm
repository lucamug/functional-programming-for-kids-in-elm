module Playground exposing
    ...
    , Number
    , animationInit_, animationSubscriptions_, animationUpdate_, animationView_
    , gameInit_, gameSubscriptions_, gameUpdate_, gameView_
    , Animation, Msg, posixToTime
    )
...
@docs animationInit_, animationSubscriptions_, animationUpdate_, animationView_
@docs gameInit_, gameSubscriptions_, gameUpdate_, gameView_
...
animation viewFrame =
    Browser.document
        { init = animationInit_
        , view =
            \(Animation _ screen time) ->
                { title = "Playground"
                , body = [ animationView_ screen time viewFrame ]
                }
        , update = animationUpdate_
        , subscriptions = animationSubscriptions_
        }


animationSubscriptions_ : Animation -> Sub Msg
animationSubscriptions_ (Animation visibility _ _) =
    case visibility of
        E.Hidden ->
...
animationUpdate_ : Msg -> Animation -> ( Animation, Cmd msg )
animationUpdate_ msg model =
    ( animationUpdate msg model
    , Cmd.none
    )


animationView_ : Screen -> a -> (a -> List Shape) -> Html.Html msg
animationView_ screen time viewFrame =
    render screen (viewFrame time)


animationInit_ : () -> ( Animation, Cmd Msg )
animationInit_ () =
...
game viewMemory updateMemory initialMemory =
    Browser.document
        { init = gameInit_ initialMemory
        , view =
            \g ->
                { title = "Playground"
                , body = [ gameView_ viewMemory g ]
                }
        , update = gameUpdate_ updateMemory
        , subscriptions = gameSubscriptions_
        }


gameView_ : (Computer -> memory -> List Shape) -> Game memory -> Html.Html msg
gameView_ viewMemory (Game _ memory computer) =
    render computer.screen (viewMemory computer memory)


gameInit_ : memory -> () -> ( Game memory, Cmd Msg )
gameInit_ initialMemory () =
    ( Game E.Visible initialMemory initialComputer
    , Task.perform GotViewport Dom.getViewport
    )


gameUpdate_ : (Computer -> memory -> memory) -> Msg -> Game memory -> ( Game memory, Cmd msg )
gameUpdate_ updateMemory msg model =
    ( gameUpdate updateMemory msg model
    , Cmd.none
    )


gameSubscriptions_ : Game memory -> Sub Msg
gameSubscriptions_ (Game visibility _ _) =
    case visibility of
        E.Hidden ->
            E.onVisibilityChange VisibilityChanged
